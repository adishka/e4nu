	...
	for (int WhichEnergy = 0; WhichEnergy < NEnergies; WhichEnergy ++) {
		...
		for (int WhichNucleus = 0; WhichNucleus < NNuclei; WhichNucleus ++) {
			...
			for (int WhichFSIModel = 0; WhichFSIModel < NFSIModels; WhichFSIModel ++) {
				...
				if (FSILabel[WhichFSIModel] == "#nu") { latex->DrawLatexNDC(0.5,0.4,FSILabel[WhichFSIModel]); }
				if (FSILabel[WhichFSIModel] == "e") { latex->DrawLatexNDC(0.53,0.25,FSILabel[WhichFSIModel]); }
				  
			} // End of the loop over the FSI Models 

			PlotCanvas->SaveAs("myPlots/"+nucleus[WhichNucleus]+"_" +E[WhichEnergy]+"_Q2Plot_OneProton_SuSav2.pdf"); 
