...
//	FSIModel.push_back("hA2018_Final_RadCorr_LFGM_SixSectors");
	...
	NameOfPlots.push_back("MissMomentum"); OutputPlotNames.push_back("DeltaPT");
	...
	TString TxtName = "myXSec/PTPanel_XSecs.txt";
	ofstream myTxtFile;
	myTxtFile.open(TxtName);
	myTxtFile << std::fixed << std::setprecision(3);
	myTxtFile << "PT Panel Ext Data Fig.5;" << endl << endl;
	...
	for (int WhichxBCut = 0; WhichxBCut < NxBCuts; WhichxBCut ++) {
		...
		TFile* fXSecFile = TFile::Open("myXSec/DeltaPT_AccCorr_XSec_"+xBCut[WhichxBCut]+".root","recreate");
					...
					for (int WhichFSIModel = 0; WhichFSIModel < NFSIModels; WhichFSIModel ++) {
						...
						if (string(FSIModelsToLabels[FSIModel[WhichFSIModel]]).find("Data") != std::string::npos) { 
							...
							if (Energy[WhichEnergy] == 4.461) { 
								...
								myTxtFile << "Pad ("+PadName+"). Cross sections scaled by 4" << endl; 
		...
		PlotCanvas->SaveAs("../../../myPlots/pdf/"+xBCut[WhichxBCut]+"/"+version+ext+"DeltaPT_FigExtData8_SuSav2_AccCorrXSec.eps");
