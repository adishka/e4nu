...
void AbsXSec_OverlayDeltaPT_FigExtData10() {
...
//			205,34,768,768);
		...
		for (int WhichPlot = 0; WhichPlot < NPlots; WhichPlot ++) {
			...
			for (int WhichEnergy = 0; WhichEnergy < NEnergies; WhichEnergy ++) {
				...
				double MaxHeight = 110; // In order to use y-axis ticks with common scale, constraint range between (0,MaxHeight)

				// Loop over the nuclei

				for (int WhichNucleus = 0; WhichNucleus < NNuclei; WhichNucleus ++) {

					if (nucleus[WhichNucleus] == "56Fe") { MaxHeight = 240; }
					...
					{ pad = new TPad(NameOfPlots[WhichPlot],NameOfPlots[WhichPlot],XMinPad,YMinPad,XMaxPad,YMaxPad, 21); }
						...
						if (DoubleE[WhichEnergy] == 2.261) 
							{ pad = new TPad(NameOfPlots[WhichPlot],NameOfPlots[WhichPlot],XMinPad-0.035,YMinPad+space,XMaxPad,YMaxPad+space, 21); }
					...
					if (DoubleE[WhichEnergy] == 2.261 && nucleus[WhichNucleus] == "56Fe") { pad->SetLeftMargin(0.11); }
...
if (DoubleE[WhichEnergy] == 4.461) { Plots[WhichFSIModel]->Scale(3); }
...
//		PlotCanvas->SaveAs("../../myPlots/pdf/"+xBCut[WhichxBCut]+"/"+version+ext+"DeltaPT_FigExtData10"+WhatModelsAreIncluded+"_SuSav2_AbsXSec.pdf");
